...
- [x] **任务**：保留并显示数据读取错误，阻止在权限数据不完整时进入可保存的编辑状态。
...
### DEV-02｜已撤销｜保留发布后的版本同步

- **用户决定（2026-09-09）**：发布后同步版本是正常操作，本项取消。已以 7ccb4921 撤销 f430b86bc06d7c42836ee3f301d39b3ef5a666ca；不再要求提交前同步或 check:versions 门禁。发布重试的相关冲突归入 DEV-01 修正。
