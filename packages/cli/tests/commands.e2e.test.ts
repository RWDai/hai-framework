...
describe('createProject — api 类型', () => {
  ...
  it('应生成 README.md', async () => {
    ...
    expect(content).toContain('## 容器运行')
    expect(content).toContain('pnpm docker:build:data')
    expect(content).toContain('pnpm podman:build:data')
    expect(content).toContain('curl --fail http://127.0.0.1:3000/health')
    expect(content).toContain('curl --fail http://127.0.0.1:3000/ready')
    expect(content).toContain('/api/v1/app/info')
    expect(content).toContain('/app/config')
    expect(content).toContain('以上三个运行时参数均可按需删除')
    expect(content).toContain('删除后使用镜像自带配置')
    expect(content).toContain('使用 `--rm` 删除容器时会丢失')
  })
  ...
  it('应生成可传入配置与嵌入数据的 API 镜像入口', async () => {
    ...
    expect(await exists(projectPath, 'apps/proj-api-service/data/.gitkeep')).toBe(true)
    expect(dockerfile).toContain('pnpm --filter proj-api-service --prod deploy')
    expect(dockerfile).toContain('target=/pnpm/store pnpm --filter proj-api-service --prod deploy')
    expect(dockerfile).toContain('COPY --from=embedded-data --chown=node:node / /app/data/')
    expect(dockerfile.indexOf('pnpm fetch --frozen-lockfile')).toBeLessThan(dockerfile.indexOf('COPY . .'))
    expect(dockerfile.indexOf('COPY . .')).toBeLessThan(dockerfile.indexOf('pnpm install --offline --frozen-lockfile --ignore-scripts'))
...
describe('createProject — admin 类型 + iam', () => {
  ...
  it('应在根 README 说明普通、带数据容器构建与验收命令', async () => {
    const readme = await readText(projectPath, 'README.md')
    expect(readme).toContain('## 容器运行')
    expect(readme).toContain('pnpm docker:build:data')
    expect(readme).toContain('pnpm podman:build:data')
    expect(readme).toContain('curl --fail http://127.0.0.1:3000/')
    expect(readme).toContain('以上三个运行时参数均可按需删除')
  })
...
describe('createProject — website 类型', () => {
  ...
  it('应生成 SvelteKit Node 容器交付文件', async () => {
    ...
    expect(dockerfile).toContain('target=/pnpm/store pnpm --filter proj-website --prod deploy')
...
describe('createProject — mobile-app 类型', () => {
  ...
  it('应生成带 SPA fallback 的静态镜像', async () => {
    ...
    const readme = await readText(projectPath, 'README.md')
    expect(readme).toContain('## 容器运行')
    expect(readme).toContain('pnpm podman:build')
    expect(readme).not.toContain('pnpm docker:build:data')
    expect(readme).toContain('curl --fail http://127.0.0.1:8080/health')
  })
...
describe('createProject — fullstack 类型', () => {
  ...
  it('应生成 pnpm workspace 多包工程', async () => {
    ...
    expect(pkg.scripts['docker:build:service']).toBeUndefined()
    expect(pkg.scripts['podman:build:service']).toBeUndefined()
    expect(pkg.scripts['docker:build:web']).toContain('--target web')
    expect(pkg.scripts['docker:build:web']).not.toContain('PUBLIC_API_BASE')
    expect(pkg.scripts['podman:build']).toContain('--jobs=1')
    expect(pkg.scripts['podman:build']).toContain('--format docker')
    expect(pkg.scripts['podman:build:data']).toContain('--jobs=1')
    ...
    expect(readme).toContain('## 容器运行')
    expect(readme).toContain('/app/config')
    expect(readme).toContain('@h-ai/serv` 直接对外监听')
    expect(readme).toContain('pnpm docker:build:data')
    expect(readme).toContain('pnpm podman:build:data')
    expect(readme).toContain('pnpm podman:build:web')
    expect(readme).toContain('curl --fail http://127.0.0.1:3000/health')
    expect(readme).toContain('curl --fail http://127.0.0.1:3000/ready')
    expect(readme).toContain('curl --fail http://127.0.0.1:8080/runtime-config.js')
    expect(readme).toContain('-e PUBLIC_API_BASE=https://api.example.com')
    expect(readme).toContain('以上三个 Service 运行时参数均可按需删除')
    expect(readme).toContain('删除后使用镜像自带配置')
    expect(readme).toContain('使用 `--rm` 删除容器时会丢失')
  ...
  it('应生成 serv 包、单元测试和可启动入口', async () => {
    ...
    expect(await exists(projectPath, 'packages/proj-fullstack-serv/data/.gitkeep')).toBe(true)
  ...
  it('应生成由 Service 直接监听的默认镜像与独立 Web 镜像', async () => {
    const dockerfile = await readText(projectPath, 'Dockerfile')
    const nginxConfig = await readText(projectPath, 'docker/nginx.conf')
    const runtimeConfig = await readText(projectPath, 'docker/runtime-config.js.template')
    const runtimeConfigEnv = await readText(projectPath, 'docker/19-runtime-config.envsh')
    const webIndex = await readText(projectPath, 'apps/proj-fullstack-web/index.html')

    expect(dockerfile).toContain('pnpm --filter proj-fullstack-serv --prod deploy')
    expect(dockerfile).toContain('target=/pnpm/store pnpm --filter proj-fullstack-serv --prod deploy')
    expect(dockerfile).toContain('FROM service AS runtime')
    expect(dockerfile).toContain('COPY --from=embedded-data --chown=node:node / /app/data/')
    expect(dockerfile.indexOf('pnpm fetch --frozen-lockfile')).toBeLessThan(dockerfile.indexOf('COPY . .'))
    expect(dockerfile.indexOf('COPY . .')).toBeLessThan(dockerfile.indexOf('pnpm install --offline --frozen-lockfile --ignore-scripts'))
    expect(dockerfile).not.toContain('ARG PUBLIC_API_BASE')
    expect(dockerfile).toContain('NGINX_ENVSUBST_OUTPUT_DIR=/tmp/hai-runtime-config')
    expect(dockerfile).toContain('USER node')
    expect(dockerfile).not.toContain('fullstack-server.mjs')
    expect(dockerfile).toContain('wget -Y off -q -O /dev/null http://127.0.0.1:8080/health')
    expect(await exists(projectPath, 'docker/fullstack-server.mjs')).toBe(false)
    expect(nginxConfig).toContain('try_files $uri $uri/ /index.html')
    expect(nginxConfig).toContain('alias /tmp/hai-runtime-config/runtime-config.js')
    expect(runtimeConfig).toContain('$' + '{PUBLIC_API_BASE_BASE64}')
    expect(runtimeConfigEnv).toContain('$' + '{PUBLIC_API_BASE:-}')
    expect(webIndex).toContain('<script src="/runtime-config.js"></script>')
    expect(webIndex).not.toContain('<script type="module" src="/runtime-config.js"></script>')
    expect(webIndex.indexOf('/runtime-config.js')).toBeLessThan(webIndex.indexOf('</head>'))
    expect(await exists(projectPath, 'apps/proj-fullstack-web/public/runtime-config.js')).toBe(true)
    expect(await exists(projectPath, '.dockerignore')).toBe(true)
  })
