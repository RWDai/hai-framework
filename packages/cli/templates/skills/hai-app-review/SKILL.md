---
name: hai-app-review
description: 对应用代码进行审查：路由安全 → 认证授权 → i18n → 组件使用 → API 端点 / service workspace → 服务层 → 性能，并补充产品体验与交互走查；当需求涉及代码审查、规范化、质量检查时使用。
---
...
| 项目 | 契约 |
| --- | --- |
| 能力 | 对应用代码进行审查：路由安全 → 认证授权 → i18n → 组件使用 → API 端点 / service workspace → 服务层 → 性能，并补充产品体验与交互走查；当需求涉及代码审查、规范化、质量检查时使用。 |
| 适用场景 | 当任务与 `hai-app-review` 的能力描述匹配，并且需要遵循本 Skill 的流程和边界时 |
